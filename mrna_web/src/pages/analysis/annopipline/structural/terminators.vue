        ...
        <el-scrollbar class="w-full" ref="scrollbarRef">
            <div class="flex flex-col h-400">
                <div class="font-600 ml-20 mt-16 flex flex-row border-b-2 w-9/10 pb-5">
                    <div class="text-4xl text-[#253959]">Transcription Terminator Annotation</div>
                    <el-button
                        round
                        size="large"
                        color="#EE7E7D"
                        class="text-white hover:text-white focus:text-white active:text-white ml-10"
                        @click="submitdemo"
                    >
                        Run Demo
                    </el-button>
...
const godemo = () => {
    router.push({ path: '/task/result/annopipline/terminator', query: { taskid: 92 } })
}
...
const submit = async () => {
    ...
    if (precheck.value) {
        submitdata.append('analysistype', 'Transcription Terminator Annotation')
